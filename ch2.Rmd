...
```{r}
tally(m111survey$sex)  #the $ tells R to look inside m111survey to find sex
```
...
```{r fig.width=3,fig.height=3}
ptabsex <- with(m111survey,tally(sex))
...
```{r}
ptabseat<- with(m111survey,tally(seat))
...
ptabseat<- with(m111survey,tally(seat))
barchart(ptabseat,horizontal=FALSE,
         main="More Likely to sit in the front: Guys vs. Gals\n(Percentages)",
         ylab="Percent")
```
...
```{r}
contTable <- xtabs(~ sex + love_first, data = m111survey)
barchart(contTable,
	auto.key = list(
		space = "top",
		title = "love_first",
